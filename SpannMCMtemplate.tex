...
\textit{About the \textbf{Missing Aircraft}}
\begin{itemize}
\item \textbf{The initial search domain is an unobstructed rectangle of ocean with a length of 500km and width of 300km.} This rectangle would cover the whole uncertainty range based on 
...
\item \textbf{The missing aircraft is assumed to be still in this domain at $t=0$.}
\item \textbf{The crashed/force landed aircraft debris is assumed to be remain close and floating at all time.} The assumption is that not enough time have passed for important
\item \textbf{The local trajectory of concern is straight}. In addition to the obvious smoothness arguments, it is always possible to apply a conformal transform on the 

\item \textbf{No banking maneuver was made from incident to crash.} This is reasonable for that even in the worse case of gliding due to single engine failure, the average time from initial to of roughly 11 minutes. no time for intentional changes on the large scale.
...
\ \\
\textit{About the\textbf{ Search Agents}}
\begin{itemize}
\item \textbf{All agents are commanded and controlled by the central planner at each update interval}.
\item \textbf{Agents arrive at the boundary of the search domain at $t=0$}.  Search agents are assumed to  edge instantaneously. a/c is in initial domain.
